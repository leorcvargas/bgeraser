...
type ImageQuery struct {
	...
	withImageProcesses *ImageProcessQuery
...
func (iq *ImageQuery) QueryImages() *ImageQuery {
	query := (&ImageClient{config: iq.config}).Query()
...
// QueryImageProcesses chains the current query on the "image_processes" edge.
func (iq *ImageQuery) QueryImageProcesses() *ImageProcessQuery {
	query := (&ImageProcessClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iq *ImageQuery) Clone() *ImageQuery {
	...
	return &ImageQuery{
		...
		withImageProcesses: iq.withImageProcesses.Clone(),
...
func (iq *ImageQuery) WithImages(opts ...func(*ImageQuery)) *ImageQuery {
	query := (&ImageClient{config: iq.config}).Query()
...
// WithImageProcesses tells the query-builder to eager-load the nodes that are connected to
// the "image_processes" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *ImageQuery) WithImageProcesses(opts ...func(*ImageProcessQuery)) *ImageQuery {
	query := (&ImageProcessClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withImageProcesses = query
	return iq
}
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	...
	if query := iq.withImages; query != nil {
		if err := iq.loadImages(ctx, query, nodes,
			func(n *Image) { n.Edges.Images = []*Image{} },
			func(n *Image, e *Image) { n.Edges.Images = append(n.Edges.Images, e) }); err != nil {
			return nil, err
		}
	}
	if query := iq.withImageProcesses; query != nil {
		if err := iq.loadImageProcesses(ctx, query, nodes,
			func(n *Image) { n.Edges.ImageProcesses = []*ImageProcess{} },
			func(n *Image, e *ImageProcess) { n.Edges.ImageProcesses = append(n.Edges.ImageProcesses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadImages(ctx context.Context, query *ImageQuery, nodes []*Image, init func(*Image), assign func(*Image, *Image)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(image.ImagesTable)
		s.Join(joinT).On(s.C(image.FieldID), joinT.C(image.ImagesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(image.ImagesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(image.ImagesPrimaryKey[0]))
...
func (iq *ImageQuery) loadImageProcesses(ctx context.Context, query *ImageProcessQuery, nodes []*Image, init func(*Image), assign func(*Image, *ImageProcess)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(imageprocess.FieldImageID)
	}
	query.Where(predicate.ImageProcess(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(image.ImageProcessesColumn), fks...))
	}))
